...
const Blog = ({ blog, callBack, setNotification, user, likeMock }) => {
  ...
  const remove = async () => {
    ...
    try {
      ...
      setNotification({ message: `Removed blog ${blog.title} by ${blog.author}`, color: 'green' })
      setTimeout(() => setNotification({ message: null }), 3000)
    } catch (exception) {
  ...
  return (
    <div className='blog' style={blogStyle}>
      ...
      {extended && (
        <>
          <div>{blog.url}</div>
          <div>{blog.likes}<button onClick={likeMock ? likeMock : like}>like</button></div>
          <div>{blog.user.name}</div>
          {blog.user.username === user.username && (<button onClick={remove} style={removeStyle}>remove</button>)}
        </>
      )}
    </div>
  )
